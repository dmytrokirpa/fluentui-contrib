...
export function useScrollToDynamicController({
  ...
  // Using native setTimeout/clearTimeout for stability checks
  // eslint-disable-next-line @typescript-eslint/no-unused-vars
  const [_setStabilityTimeoutFn, _clearStabilityTimeoutFn] = useTimeout();
  ...
  const clearStabilityTimeout = React.useCallback(
    (operation: ScrollOperation) => {
      ...
      view?.clearTimeout(operation.stabilityTimeoutId);
  ...
  scheduleStabilityCheckRef.current = (operation: ScrollOperation) => {
    ...
    if (canReschedule) {
      ...
      const delay = operation.awaitingSmoothScroll
        ...
        : stabilityTimeout;
      const view = getDefaultView();
      const timeoutId = view?.setTimeout(runCheck, delay);
      operation.stabilityTimeoutId = timeoutId as unknown as number;
    } else {
      operation.stabilityTimeoutId = null;
      runCheck();
    }
