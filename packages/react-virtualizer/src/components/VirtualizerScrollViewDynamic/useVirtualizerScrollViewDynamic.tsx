...
import { useScrollToDynamicController } from '../../hooks/useScrollToDynamicController';
...
export function useVirtualizerScrollViewDynamic_unstable(
  ...
  const sizeTrackingArray = React.useRef<number[]>(
    Array.from({ length: props.numItems }, () => props.itemSize)
  );
  ...
  // Track whether a scrollTo operation is active to disable useMeasureList's scroll compensation
  ...
  const measureObject = useMeasureList({
    ...
    disableScrollCompensationRef: isScrollToActiveRef,
  });
  ...
  const handleOperationComplete = React.useCallback(
    (index: number) => {
      // Don't disable scroll compensation here - wait until pin is released
      // The controller handles compensation while pinned
      resolveScrollCallback(index);
    },
    [resolveScrollCallback]
  );
  ...
  const handlePinReleased = React.useCallback((_index: number) => {
    void _index;
    // Re-enable useMeasureList scroll compensation now that the operation is fully complete
    isScrollToActiveRef.current = false;
  }, []);
  ...
  React.useImperativeHandle(
    ...
    () => ({
      ...
      scrollTo(
        ...
        startScrollTo(index, behavior, callback);
      },
